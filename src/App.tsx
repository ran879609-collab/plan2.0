...
import { AppStorage, CloudSyncStatus } from './utils/storage';
import { authService } from './services/authService';
import { migrationService, LocalDataSummary } from './services/migrationService';
...
export default function App() {
  ...
  const [cloudSyncStatus, setCloudSyncStatus] = useState<CloudSyncStatus>(AppStorage.getSyncStatus());
  ...
  const [isGuestMode, setIsGuestMode] = useState<boolean>(() => {
    return localStorage.getItem('wmt_guest_mode_preferred') === 'true';
  });
  ...
  useEffect(() => {
    const unsub = AppStorage.onSyncStatusChange((status) => {
      setCloudSyncStatus(status);
    });
    return () => unsub();
  }, []);

  // Check auth session & load user cloud data
  useEffect(() => {
    let mounted = true;
    ...
    async function checkAuthAndLoad() {
      ...
      try {
        ...
        if (session?.user && mounted) {
          ...
          AppStorage.setCurrentUserId(session.user.id);
          try {
            await AppStorage.loadFromCloud(session.user.id);
            reloadData();

            // Detect if local unmigrated data exists
            const localSummary = migrationService.detectLocalData();
            if (localSummary.hasData && !localSummary.alreadyMigrated) {
              setMigrationSummary(localSummary);
              setIsMigrationModalOpen(true);
            }
          } catch (cloudErr) {
            console.warn('Could not load from cloud, fallback to local cache:', cloudErr);
            reloadData();
          }
        } else {
          reloadData();
    ...
    const { data: authSub } = authService.onAuthStateChange(async (event, session) => {
      ...
      if (session?.user) {
        ...
        try {
          ...
          console.warn('Auth change cloud load failed:', cloudErr);
          reloadData();
        }
      } else {
        setCurrentUser(null);
  ...
  const handleExecuteResetDemo = async () => {
    await AppStorage.resetToDemoData(settings.customTodayDate);
    reloadData();
    showToast('⚡ 沃尔玛业务参考数据已成功加载并同步至云端（包含 8 大典型业务场景与真实货件）');
  };
  ...
  const handleExecuteClearAllData = async () => {
    ...
    reloadData();
    showToast('🗑️ 已清空当前账号所有业务数据，系统已重置为空白就绪状态');
  };
  ...
  const handleTriggerCloudSync = async () => {
    ...
    try {
      ...
      await AppStorage.loadFromCloud(currentUser.id);
      reloadData();
      showToast('☁ 已成功从云端重新拉取并同步最新业务数据');
    } catch (err: any) {
      showToast(`云端拉取异常: ${err.message || '网络不稳定'}`);
    }
  };

  const checkAndPromptMigration = () => {
    const summary = migrationService.detectLocalData();
    if (summary.hasData) {
      setMigrationSummary(summary);
      setIsMigrationModalOpen(true);
    } else {
      showToast('本地未检测到需要迁移的遗留数据');
    }
  };
  ...
  const handleImportJsonBackup = (jsonContent: string) => {
    ...
    if (success) {
      showToast('备份数据导入恢复成功！');
      reloadData();
    } else {
      showToast('备份文件格式解析失败，请检查 JSON 结构');
    }
  };
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-slate-100 text-slate-900 font-sans antialiased relative">
        ...
        <main className="flex-1 overflow-y-auto bg-slate-100/70">
          ...
          {currentTab === 'settings' && (
            <SettingsView
              ...
              user={currentUser ? { email: currentUser.email, id: currentUser.id } : null}
              syncStatus={cloudSyncStatus}
              onTriggerCloudSync={handleTriggerCloudSync}
              onTriggerMigrationModal={checkAndPromptMigration}
            />
          )}
        </main>
      </div>
      ...
      {/* 10. Supabase Authentication Modal */}
      {isAuthModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in">
          <div className="relative w-full max-w-md">
            <button
              onClick={() => setIsAuthModalOpen(false)}
              className="absolute top-4 right-4 z-10 p-2 text-slate-400 hover:text-slate-600 bg-white/90 hover:bg-white rounded-full shadow-xs transition-colors"
              title="关闭"
            >
              <X className="w-4 h-4" />
            </button>
            <AuthView
              onAuthSuccess={(user) => {
                ...
                localStorage.removeItem('wmt_guest_mode_preferred');
                showToast(`欢迎回来，${user?.email || '用户'}`);
              }}
              onContinueAsGuest={() => {
                setIsAuthModalOpen(false);
                setIsGuestMode(true);
                localStorage.setItem('wmt_guest_mode_preferred', 'true');
                showToast('已切换至本地演示模式');
              }}
            />
          </div>
        </div>
      ...
      {/* 11. Data Migration Modal */}
      {isMigrationModalOpen && currentUser && migrationSummary && (
        <MigrationModal
          ...
          onMigrationComplete={() => {
            ...
            reloadData();
            showToast('🎉 本地数据已成功合并并持久化至 Supabase 云端数据库！');
          }}
        />
      )}
    </div>
