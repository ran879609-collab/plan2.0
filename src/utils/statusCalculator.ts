...
export function deriveShipmentStatus(
  ...
  if (initialCaseStatus === 'Resolved' || initialCaseStatus === 'Closed' || initialStatus === 'Resolved' || initialStatus === 'Closed') {
    status = 'Resolved';
    ...
    if (caseStatus === 'Opened' || caseStatus === 'In Review' || initialStatus === 'Case Processing' || initialStatus === 'Case Opened') {
  ...
  } else if (arrivalDate || initialStatus === 'Arrived' || initialStatus === 'Receiving') {
    // Arrived at warehouse, awaiting receiving / check-in
    if (caseStatus === 'Opened' || caseStatus === 'In Review' || initialStatus === 'Case Processing' || initialStatus === 'Case Opened') {
      status = 'Case Processing';
    } else if (caseStatus === 'Eligible') {
      status = 'Case Eligible';
    } else if (initialStatus === 'Receiving') {
      status = 'Receiving';
    } else {
      status = 'Arrived';
    }
  } else if (!shipDate && totalShipQty === 0) {
    status = 'Draft';
  } else if (initialStatus && initialStatus !== 'Draft' && initialStatus !== 'In Transit') {
    status = initialStatus;
  } else {
    status = 'In Transit';
  }
...
export function calculateInventoryItemMetrics(
  ...
  shipments.forEach((shp) => {
    // Filter all matching items for this SKU to accurately handle repeat SKU entries (e.g. standalone + mixed box)
    const matchingItems = shp.items?.filter(
    ...
    matchingItems.forEach((matchedItem) => {
      if (!isArrived) {
        // 货件已创建但还未到仓 -> 全部发货数累加至在途
        inboundQty += Number(matchedItem.shipQty) || 0;
      } else {
        // 货件已到仓 -> 未上架完成的部分 (shipQty - receivedQty)
        const sQty = Number(matchedItem.shipQty) || 0;
        const rQty = Number(matchedItem.receivedQty) || 0;
        const diff = Math.max(0, sQty - rQty);
