      ...
      try {
        ...
        rawJson.forEach((row, idx) => {
          ...
          if (existIdx >= 0) {
            ...
            const prevBreakdown = prev.packageBreakdown || `${prev.shipQty}件单箱${prev.cartons}箱`;
            const currBreakdown = `${rawShipQty}件单箱${cartons}箱`;
            prev.packageBreakdown = `${prevBreakdown}, ${currBreakdown}`;
            ...
            warnings.push({
              row: rowNum,
              field: 'Duplicate SKU',
              message: `第 ${rowNum} 行 SKU ${rawSku} 重复出现，已自动累加合并数量（总计: ${items[existIdx].shipQty}件）并记录装箱拆分`,
            });
          } else {
            items.push({
              shipmentId: rawShipmentId,
