...
export const FreightSyncModal: React.FC<FreightSyncModalProps> = ({
  ...
  const handleConfirmSync = () => {
    ...
    selectedGroups.forEach((g) => {
      ...
      // Group freight items by SKU to handle repeat SKU entries within the same shipment
      // (e.g. 70 units standalone 1 box + 20 units mixed 1 box = total 90 units with clear packaging breakdown)
      const skuGroups = new Map<string, typeof g.items>();
      g.items.forEach((it) => {
        const skuKey = (it.sku || '').trim().toLowerCase();
        const list = skuGroups.get(skuKey) || [];
        list.push(it);
        skuGroups.set(skuKey, list);
      });

      const mappedItems: ShipmentItem[] = [];

      skuGroups.forEach((itemList, skuKey) => {
        ...
        // Resolve title
        let resolvedTitle = first.productName;
        if (!resolvedTitle || resolvedTitle.trim() === '' || resolvedTitle === first.sku || resolvedTitle === '产品名称') {
          resolvedTitle = existingItem?.productName || invItem?.productName || prodItem?.productName || first.productName || first.sku;
        }
        ...
        // Sum quantities and cartons across all appearances of this SKU in this shipment
        const totalShipQty = itemList.reduce((sum, it) => sum + (Number(it.actualQty) || 0), 0);
        const totalCartons = itemList.reduce((sum, it) => sum + (Number(it.boxCount) || 0), 0);
        const qtyPerCarton = totalCartons > 0 ? Math.round(totalShipQty / totalCartons) : totalShipQty;

        const receivedQty = existingItem?.receivedQty || 0;
        const receivedCartons = existingItem?.receivedCartons || 0;
        const discrepancyQty = Math.max(0, totalShipQty - receivedQty);

        // Generate packaging breakdown annotation: e.g. "70件单箱1箱, 20件混箱(mix01)1箱"
        const breakdownParts: string[] = [];
        itemList.forEach((it) => {
          const qty = Number(it.actualQty) || 0;
          const boxes = Number(it.boxCount) || 1;
          if (it.mixedBoxGroup && it.mixedBoxGroup.trim()) {
            breakdownParts.push(`${qty}件混箱(${it.mixedBoxGroup.trim()})${boxes}箱`);
          } else {
            breakdownParts.push(`${qty}件单箱${boxes}箱`);
          }
        });

        const packageBreakdown =
          breakdownParts.length > 1 || itemList.some((it) => !!it.mixedBoxGroup)
            ? breakdownParts.join(', ')
            : undefined;
      ...
      if (existing) {
        ...
        const daysPassed = shipDateStr ? calculateDaysDifference(shipDateStr, getTodayString()) : 0;
        
        let initialStatus: 'In Transit' | 'Arrived' | 'Fully Received' = 'In Transit';
        let arrivalDate: string | undefined = undefined;
        let eta: string | undefined = addDaysToDate(shipDateStr, 14);
        let recUnits = 0;
        let recCartons = 0;
        ...
        if (daysPassed >= 25) {
          ...
          arrivalDate = addDaysToDate(shipDateStr, 14);
          recUnits = 0;
          recCartons = 0;
        } else {
          // Shipped recently -> Currently in transit
          initialStatus = 'In Transit';
          recUnits = 0;
          recCartons = 0;
        }

        const itemsWithStatus = mappedItems.map(item => {
          if (initialStatus === 'Fully Received') {
            return {
              ...item,
              receivedQty: item.shipQty,
              receivedCartons: item.cartons,
              discrepancyQty: 0,
            };
          }
          return item;
              ...
              <div className="space-y-2">
                ...
                <div className="border border-slate-200 rounded-lg overflow-hidden max-h-60 overflow-y-auto divide-y divide-slate-100">
                  {groupedShipments.map((g) => {
                    ...
                    const distinctSkus = Array.from(new Set(g.items.map((it) => (it.sku || '').trim().toLowerCase())));
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            {/* SKU & Title preview line with repetition breakdown */}
                            <div className="text-[10px] text-slate-500 mt-1 truncate max-w-md">
                              SKU:{' '}
