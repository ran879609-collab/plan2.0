...
export const ProductSupplementModal: React.FC<ProductSupplementModalProps> = ({
  ...
  const handleFieldChange = (index: number, field: keyof ShipmentItem, val: any) => {
    ...
    if (field === 'shipQty' || field === 'cartons') {
      ...
      // Auto calculate receivedCartons: e.g. 72 units in 2 boxes (36/box) -> 36 = 1 box, 72 = 2 boxes
      if (sQty > 0 && ctn > 0) {
        const singleBoxQty = sQty / ctn;
        if (rQty === 0) {
          current.receivedCartons = 0;
  ...
  // Save All
  const handleSave = () => {
    // Consolidate duplicate SKUs within the shipment if any
    ...
    items.forEach((it) => {
      ...
      if (!existing) {
        consolidatedMap.set(k, { ...it, shipmentId: shipment.id });
      } else {
    ...
    onSaveItems(shipment.id, Array.from(consolidatedMap.values()));
    onClose();
  };
