...
export const SkuDetailDrawer: React.FC<SkuDetailDrawerProps> = ({
  ...
  linkedShipments.forEach((s) => {
    const matchingItems = s.items?.filter((it) => (it.sku || '').toLowerCase() === sku.toLowerCase()) || [];
    matchingItems.forEach((item) => {
      totalShipped += Number(item.shipQty) || 0;
      totalReceived += Number(item.receivedQty) || 0;
      totalDiscrepancy += Number(item.discrepancyQty) || 0;
    });
  });
              ...
              <table className="w-full text-xs text-left">
                ...
                <tbody className="divide-y divide-slate-200">
                  {linkedShipments.map((shp) => {
                    const matchingItems = shp.items?.filter((i) => (i.sku || '').toLowerCase() === sku.toLowerCase()) || [];
                    const itShipQty = matchingItems.length > 0
                      ? matchingItems.reduce((sum, i) => sum + (Number(i.shipQty) || 0), 0)
                      : shp.totalShipQty;
                    const itReceivedQty = matchingItems.length > 0
                      ? matchingItems.reduce((sum, i) => sum + (Number(i.receivedQty) || 0), 0)
                      : shp.totalReceivedQty;
                    const itDiscrepancyQty = matchingItems.length > 0
                      ? matchingItems.reduce((sum, i) => sum + (Number(i.discrepancyQty) || 0), 0)
                      : shp.totalDiscrepancyQty;
                    const breakdownTexts = matchingItems
                      .map((i) => i.packageBreakdown)
                      ...
                      <tr key={shp.id} className="hover:bg-slate-50">
                        <td className="p-2.5">
                          ...
                          {breakdownTexts.length > 0 && (
                            <div className="text-[10px] text-amber-700 bg-amber-50 px-1.5 py-0.5 rounded border border-amber-200 mt-1 max-w-[220px] truncate" title={breakdownTexts.join('; ')}>
                              📦 {breakdownTexts.join('; ')}
                            </div>
                          )}
                        </td>
                        ...
                        <td className="p-2.5 text-right font-mono font-semibold text-slate-800">{itShipQty}</td>
                        ...
                        >
                          {itDiscrepancyQty > 0 ? `-${itDiscrepancyQty}` : '0'}
                        </td>
